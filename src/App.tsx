...
const App: React.FC = () => {
  ...
  const ChangeProjectStateClick = (event: any) => {
    ...
    if (changeTo === BUTTON_LABEL_TEXTS.finish) {
      selectedProjects.map((project) =>
        updateProjectState(project, PROJECT_STATES.finished)
      );
    }

    if (changeTo === BUTTON_LABEL_TEXTS.launch) {
      selectedProjects.map((project) =>
        updateProjectState(project, PROJECT_STATES.launched)
      );
    }
  ...
  const buttonFinish = {
    label: BUTTON_LABEL_TEXTS.finish,
    onClick: ChangeProjectStateClick,
  };
  const buttonLaunch = {
    label: BUTTON_LABEL_TEXTS.launch,
    onClick: ChangeProjectStateClick,
  };
      ...
      <div className='landing'>
        <div className='dashboard'>
          {projects.map((project) => (
            <Project
              ...
              {...project}
              stateClass={project.status.toLowerCase().replace(' ', '-')}
              isHighlighted={isHighlighed.includes(project.id)}
              isClickable={
                (project.status === selectedState ||
                  selectedState === PROJECT_STATES.empty) &&
                project.status !== PROJECT_STATES.finished
              }
            />
          ))}
        </div>
        <div className='controls'>
          <Button
            {...buttonLaunch}
            disabled={
              selectedState === PROJECT_STATES.launched ||
              selectedState === PROJECT_STATES.empty
            }
          ></Button>
          <Button
            {...buttonFinish}
            disabled={
              selectedState === PROJECT_STATES.notStarted ||
              selectedState === PROJECT_STATES.empty
            }
          ></Button>
        </div>
